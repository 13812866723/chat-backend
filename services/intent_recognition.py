    ...
    def register(self, intent: str):
        ...
        def decorator(func):
            if intent not in self.handlers:
                self.handlers[intent] = []
            self.handlers[intent].append(func)
            return func
        return decorator
    
    def route(self, intent_result: Dict, *args, **kwargs):
        """根据意图路由到相应处理器"""
        ...
        if intent in self.handlers:
            ...
            if confidence >= 0.7:
                ...
                for handler in self.handlers[intent]:
                    result = handler(intent_result, *args, **kwargs)
                ...
                return {
                    "status": "confirm",
                    "message": f"我理解您可能想{intent_result.get('reasoning', '')}，对吗？"
                }
        
        # 无匹配处理器或低置信度
        return {
            "status": "fallback",
            "message": "抱歉，我不太理解您的意图。"
        }
